...
**绝大多数用户只需要以下三步，无需开启本地代理：**

1. **添加供应商**：点击"添加供应商" → 选择预设或填写 API Key
2. **切换供应商**：主界面点击"启用"，或通过系统托盘直接切换
3. **重启 CLI**：Codex 需重启终端，Claude/Gemini 即时生效
...
|------|-----------------|
| 日常切换供应商 | ❌ 不需要 |
| 自动故障转移（主供应商故障时自动切换备用） | ✅ 需要 |
| 用量监控（Token 统计、费用估算） | ✅ 需要 |
| 请求日志（排查问题、审计记录） | ✅ 需要 |

> 💡 **关于设置里的「在主页面显示本地代理开关」**：这只是控制主页面顶部是否显示代理快捷按钮，不会自动启动代理服务。如果你不使用代理功能，保持默认关闭即可。
